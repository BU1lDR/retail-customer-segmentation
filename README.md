...
| [tools/check_docs_drift.py](tools/check_docs_drift.py) | Holds the README, the report's fact names, the notebook's cell sources, and every anchored figure in the notebook's prose, its committed outputs, the `.docx` and the profile blurb in `.github/PROFILE.md` to `outputs/facts.json`. Run in CI. |
