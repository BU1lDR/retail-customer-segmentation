...
Only the `.docx` is generated from `outputs/facts.json`. This README quotes the
same numbers by hand, and the notebook's prose does too, so both can go stale
against a re-run while still reading as authoritative. So the check holds all
three to `facts.json`: 39 numbers in this README, all 186 facts
`build_report.py` reads by name, and the notebook against its builder.

It needs no source data and no notebook run -- it reads only committed files,
which is why it can run on every push. It earned its keep on the first run by
finding a figure in the notebook's prose -- "20 off-peak cohorts" where
`facts.json` says 18 -- that the generated report had had right the whole time.
...
| [tools/check_docs_drift.py](tools/check_docs_drift.py) | Holds the README, the report's fact names and the notebook to `outputs/facts.json`. Run in CI. |
